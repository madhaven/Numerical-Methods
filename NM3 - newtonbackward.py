...
while(True):
    try: f=float(input('\nEnter value of x : '))
        ...
        if f=='': exit()
    p=(f-list(thedict.keys())[n-1])/h
    print('x :', f); print('x%d :'%(n-1), list(thedict.keys())[n-1]); print('h :', h); print('p :', p);print()

    fx=list(thedict.values())[n-1]; print(fx)
    for x in range(1, n-1):
        thep=1
        for y in range(x):
            thep*=(p+y)
        print(round(thep*delta[x-1][len(delta[x-1])-1]/fact(x), prec))
        fx+=thep*delta[x-1][len(delta[x-1])-1]/fact(x)
